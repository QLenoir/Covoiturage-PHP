...
<?php
	...
	if(empty($_POST['per_pwd'])) { ?>
		<form action="index.php?page=11" method="post">
			<p><label>Nom d'utilisateur : </label></p><input type="text" id="champ" name="per_login" size="4" required>
			<p><label>Mot de passe : </label></p><input type="password" id="champ" name="per_pwd" size="4" required>
			<?php 
			...
			<div id="captcha"><img id="nb" src=<?php echo "image/nb/".$_SESSION['src1'] ?> alt=Captcha title="Captcha"  /> <b id ="operation">+</b> 
			   ...
			   <input type="text" id="champ" name="reponse" size="4" required>
	...
	<?php 
		...
		if($log === 1) {
			...
			<?php } ?>
			<form action="index.php?page=11" method="post">
			<label>Nom d'utilisateur : </label><input type="text" id="champ" name="per_login" size="4" required>
			<p><label>Mot de passe : </label><input type="password" id="champ" name="per_pwd" size="4" required></p><br>
			<?php 
				$_SESSION['src1'] = rand(1, 9);
				$_SESSION['src2'] = rand(1, 9);
			?>
